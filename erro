...
with st.sidebar:
    ...
    st.caption("Se encontrado na ementa ou indexação → índice 1 automático")
...
if run_btn:
    ...
    for idx in range(total):
        ...
        # Termos de exclusão — se encontrado → força índice 1
        if exclude_terms.strip() and s > 1:
            ...
            if excl_hits:
                s = 1
                j = f"Excluído — termo de exclusão encontrado: {', '.join(excl_hits[:5])}."
