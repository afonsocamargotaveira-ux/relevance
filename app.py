...
def score_relevance(ementa: str, indexacao: str, theme: str, extra_raw: str) -> tuple:
    ...
    if not all_terms:
        return 1, "Nenhum termo válido informado."
...
with st.sidebar:
    ...
    main_theme = st.text_input(
        "Tema", placeholder='Ex.: pessoa com deficiência  ou  deficien*',
...
if run_btn:
    ...
    col_tema_auto   = find_col(header, ["tema"])   # coluna Tema também é relevante
    if col_ementa_auto is None:
        col_ementa_auto = find_col(header, ["descricao","objeto","assunto","resumo","teor"])
    ...
    with st.expander("⚙️ Configurar colunas (clique se os resultados parecerem errados)", expanded=False):
    ...
    with st.expander(f"🔍 Termos interpretados — {len(tt_show)} do tema + {len(et_show)} adicionais"):
    ...
    for idx in range(total):
        ...
        # Concatenar todos os campos de texto relevantes
